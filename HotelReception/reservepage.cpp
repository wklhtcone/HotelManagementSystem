...
void reservepage::on_pushButton_clicked()
{
    ...
    if(flag)
        ...
        message.kind=_check_in_directly;
    QString temp;
    switch (type)
    {
        case 0:
    ...
    //收包
    DataStruct data;
    while(clientConnector.getMessage(data)==0||(data.kind!=_reservation&&data.kind!=_check_in_directly))
    {
        count++;
        Sleep(1000);
        if(count>5) break;
...
void reservepage::paintEvent(QPaintEvent *)
{
    ...
    //加载背景图
    QPixmap pix3;
    pix3.load("../hotelmanagement/555.png");
...
void reservepage::setflag(bool f)
{
    flag=f;
}
